"""B-legal mirrored GEMM+bias+ReLU expert for the capability-gate study."""
    ...
    def kernel(
        ...
        with T.Kernel(
            ...
            for row, column in T.Parallel(BLOCK_M, BLOCK_N):
                value = accumulator[row, column] + T.cast(
                    bias[block_n * BLOCK_N + column], T.float32
                )
                accumulator[row, column] = T.max(value, T.float32(0.0))
