"""Trusted Triton implementation for static FP16 GEMM+bias+ReLU."""
...
def _gemm_bias_relu_kernel(
    ...
    bias = tl.load(bias_pointer + offsets_n).to(tl.float32)
    result = tl.maximum(accumulator + bias[None, :], 0.0)
    output_offsets = offsets_m[:, None] * N_SIZE + offsets_n[None, :]
    tl.store(output_pointer + output_offsets, result)
