"""Trusted TileLang implementation for static FP16 LayerNorm."""
...
def _build_kernel():
    ...
    def kernel(
        ...
        beta: T.Tensor((COLUMNS,), T.float16),
        ...
        with T.Kernel(ROWS, threads=256) as row:
            ...
            beta_shared = T.alloc_shared((COLUMNS,), T.float16)
            ...
            mean = row_sum[0] / COLUMNS
            variance = square_sum[0] / COLUMNS - mean * mean
            inverse_std = T.rsqrt(variance + EPSILON)
            for _, column in T.Parallel(1, COLUMNS):
                normalized = (x_local[0, column] - mean) * inverse_std
                x_shared[0, column] = T.Cast(
                    T.float16,
                    normalized * T.Cast(T.float32, gamma_shared[column])
                    + T.Cast(T.float32, beta_shared[column]),
                )
...
class ModelNew(nn.Module):
    ...
    def forward(
        ...
        return self.kernel(x, gamma, beta)
